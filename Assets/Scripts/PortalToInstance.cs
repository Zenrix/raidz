...
public class PortalToInstance : MonoBehaviour
{
    ...
    void OnPortal(Player player)
    {
        ...
        if (player.InParty())
        {
            ...
            if (instanceTemplate.instances.TryGetValue(player.party.partyId, out existingInstance))
            {
            ...
            else
            {
                ...
                else if (player.isServer) player.chat.TargetMsgInfo(player.connectionToClient, "There are already too many " + instanceTemplate.name + " instances. Please try again later.");
            }
        }
    }
