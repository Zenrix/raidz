    ...
    void Update()
    {
        ...
        if (player)
        {
            ...
            if (panel.activeSelf)
            {
                ...
                noticeEditButton.onClick.SetListener(() => { noticeInput.interactable = true; });
                ...
                leaveButton.onClick.SetListener(() => { player.CmdLeaveGuild(); });
                ...
                for (int i = 0; i < memberCount; ++i)
                {
                    ...
                    slot.promoteButton.onClick.SetListener(() => { player.CmdGuildPromote(member.name); });
                    slot.demoteButton.interactable = guild.CanDemote(player.name, member.name);
                    slot.demoteButton.onClick.SetListener(() => { player.CmdGuildDemote(member.name); });
                    slot.kickButton.interactable = guild.CanKick(player.name, member.name);
                    slot.kickButton.onClick.SetListener(() => { player.CmdGuildKick(member.name); });
                }
            }
        }
