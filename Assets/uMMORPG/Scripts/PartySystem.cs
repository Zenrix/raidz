...
public static class PartySystem
{
    ...
    // copy part to someone
    static void CopyTo(string member, Party party)
    {
        Player player;
        if (Player.onlinePlayers.TryGetValue(member, out player))
            player.party = party;
    }

    // copy party to all members
    static void CopyToMembers(Party party)
    {
    ...
    public static void LeaveParty(int partyId, string member)
    {
        ...
        if (parties.TryGetValue(partyId, out party))
        {
            ...
            {
                // remove from list
                List<string> list = party.members.ToList();
                list.RemoveAll(name => name == member);
                party.members = list.ToArray();
                ...
                if (party.members.Length > 1)
                {
                    // broadcast and save in dict
                    CopyToMembers(party);
                    CopyTo(member, new Party()); // clear for kicked person
                    parties[partyId] = party;
                }
                ...
                else
                {
                    ...
                    CopyTo(party.members[0], new Party()); // clear for master
                    CopyTo(member, new Party()); // clear for kicked person
